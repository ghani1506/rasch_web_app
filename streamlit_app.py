...
# -----------------------------
# Single and multidimensional analysis
# -----------------------------
...
# -----------------------------
# Non-overlapping person-item map helper
# -----------------------------

def spread_label_positions(y_values, min_gap=0.18):
    """
    Creates non-overlapping y-positions for item labels.
    The item points stay at their true logits; only the text labels are spread.
    """
    ...
    sorted_y = y_values[order].copy()
    adjusted = sorted_y.copy()

    for i in range(1, len(adjusted)):
        if adjusted[i] - adjusted[i - 1] < min_gap:
            adjusted[i] = adjusted[i - 1] + min_gap

    original_mid = np.mean(sorted_y)
    adjusted_mid = np.mean(adjusted)
    adjusted = adjusted - (adjusted_mid - original_mid)

    label_positions = np.empty_like(adjusted)
    label_positions[order] = adjusted
...
def make_person_item_map(theta, difficulty, item_names):
    """
    Improved person-item map:
    - Wider figure
    - Item labels placed on a separate label column
    - Labels are automatically spread to reduce overlap
    - Grey guide lines connect true item positions to label positions
    """
    fig, ax = plt.subplots(figsize=(10, 8))

    theta = np.asarray(theta, dtype=float)
    difficulty = np.asarray(difficulty, dtype=float)
    item_names = [str(x) for x in item_names]
    ...
    label_y = spread_label_positions(difficulty, min_gap=0.20)

    ax.scatter(x_person, theta, alpha=0.22, s=14, label="Persons")
    ax.scatter(np.full(len(difficulty), item_x), difficulty, marker="D", s=70, label="Items")

    for true_y, text_y, label in zip(difficulty, label_y, item_names):
        ax.plot([item_x + 0.03, label_x - 0.02], [true_y, text_y], linewidth=0.7, alpha=0.45)
        ax.text(
            label_x,
            text_y,
            label,
            ...
            ha="left",
            fontsize=8,
            bbox=dict(boxstyle="round,pad=0.18", facecolor="white", edgecolor="lightgray", alpha=0.9),
        )

    y_min = min(np.min(theta), np.min(difficulty), np.min(label_y)) - 0.6
    y_max = max(np.max(theta), np.max(difficulty), np.max(label_y)) + 0.6

    ax.set_ylim(y_min, y_max)
    ax.set_xlim(-0.35, 1.75)
    ax.set_xticks([0, item_x])
...
def generate_pdf_report(
    ...
    for fig in figs:
        ...
        story.append(Image(tmp_path, width=8.4 * inch, height=5.5 * inch))
        story.append(Spacer(1, 0.15 * inch))
...
with st.expander("CSV format examples"):
    ...
    st.code(
        "student,item_1,item_2,item_3,item_4,item_5,item_6\n"
        "P1,1,1,0,0,2,3\n"
        "P2,1,0,0,0,1,2\n"
        "P3,1,1,1,0,3,3",
        language="csv",
...
if uploaded_file is not None:
    ...
    if "analysis" in st.session_state:
        ...
        sorted_difficulty = sorted_results["difficulty_logit"].values
        sorted_item_names = list(sorted_results["item"])

        fig1 = make_distribution_plot(theta, sorted_difficulty)
        st.pyplot(fig1)
        plt.close(fig1)

        fig2 = make_person_item_map(theta, sorted_difficulty, sorted_item_names)
        st.pyplot(fig2)
        plt.close(fig2)
